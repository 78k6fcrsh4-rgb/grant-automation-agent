...
Env:
  ...
  SEED_USERS = JSON list, e.g.
     [{"email":"jane@dupagehealth.org","password":"...","name":"Jane Doe","role":"member"}]
...
def ensure_user(*, tenant_name: str, slug: str, email: str, password: str,
                role: str = "member", full_name: Optional[str] = None) -> str:
...
def seed_users_from_env() -> List[Tuple[str, str]]:
    ...
    for u in users:
        ...
        status = ensure_user(
            ...
            role=u.get("role", "member"), full_name=u.get("name"),
        )
